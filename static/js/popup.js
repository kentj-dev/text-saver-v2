...
const contextSettingsButton = document.getElementById('context-settings');
const contextSecurityButton = document.getElementById('context-security');
const contextSecurityIcon = document.getElementById('context-security-icon');
const contextSecurityLabel = document.getElementById('context-security-label');
...
function startGuide() {
  ...
  tourDriver = createDriver({
    ...
    steps: [
      ...
      {
        element: '.tabs-toolbar',
        popover: {
          title: 'Organize with tabs',
          description: 'Click to switch, drag to reorder, or right-click a tab for settings and security actions.',
          side: 'bottom',
          align: 'start',
        },
      },
      {
        ...
        popover: {
          title: 'Create a tab',
          description: `Add up to ${MAX_USER_TABS} saved-text tabs.`,
          side: 'left',
          align: 'start',
        },
      },
      {
        ...
        popover: {
          ...
          description: 'Your text saves locally as you type. Wrapped rows stay aligned with logical line numbers.',
          side: 'top',
          align: 'center',
        },
      },
      {
        element: '#line-numbers:not([hidden])',
        popover: {
          title: 'Copy individual lines',
          description: 'Hover over a line number and click its copy icon to copy only that line.',
          side: 'right',
          align: 'start',
        },
      },
      {
        ...
        popover: {
          title: 'Quick actions',
          description: 'Switch theme, search the active tab, protect it with a password, rename it, or delete it.',
          side: 'bottom',
          align: 'end',
        },
      },
      {
        ...
        popover: {
          title: 'Backups and help',
          description: 'Export or import backups, visit the developer, or replay this guide from the menu.',
          side: 'left',
          align: 'end',
        },
      },
      {
        ...
        popover: {
          ...
          description: 'Track words, characters, and lines, then download or copy the active tab.',
          side: 'top',
          align: 'center',
        },
      },
...
async function startFirstRunGuide() {
  const stored = await chrome.storage.local.get(GUIDE_SEEN_KEY);
  if (stored[GUIDE_SEEN_KEY]) return;
...
function openModal({ title, message, confirmLabel = 'Confirm', input, inputTwo, preview, options, validate }) {
...
tabsContainer.addEventListener(
  ...
  (event) => {
    ...
    const canScroll =
      (delta < 0 && tabsContainer.scrollLeft > 0) || (delta > 0 && tabsContainer.scrollLeft < maxScrollLeft);
    if (!canScroll) return;
    event.preventDefault();
...
function showApproachingLimitWarning() {
  ...
  const nearCharacters =
    text.length >= activePlan.maxCharactersPerTab * LIMIT_WARNING_RATIO && text.length < activePlan.maxCharactersPerTab;
  const nearLines =
    lineCount >= activePlan.maxLinesPerTab * LIMIT_WARNING_RATIO && lineCount < activePlan.maxLinesPerTab;
  const warnings = [];
...
function renderTabs() {
  ...
  state.tabs.forEach((tab) => {
    ...
    if (isEncryptedTab(tab)) {
      ...
      lockIcon.src = isUnlocked(tab) ? 'static/svgs/unlock.svg' : 'static/svgs/lock.svg';
      lockIcon.alt = '';
...
function closeTabContextMenu({ returnFocus = false } = {}) {
  if (tabContextMenu.hidden) return;
  tabContextMenu.hidden = true;
  if (returnFocus && contextTabId) {
    tabsContainer.querySelector(`[data-tab-id="${CSS.escape(contextTabId)}"] .tab-select`)?.focus();
  }
...
function openTabContextMenu(tabId, clientX, clientY) {
  ...
  contextSettingsButton.disabled = reserved;
  contextDeleteButton.disabled = reserved || normalTabCount() === 1;
  contextSecurityButton.disabled = reserved;
  if (!isEncryptedTab(tab)) {
    contextSecurityLabel.textContent = 'Set password';
    contextSecurityIcon.src = 'static/svgs/lock.svg';
  } else if (isUnlocked(tab)) {
    contextSecurityLabel.textContent = 'Lock now';
    contextSecurityIcon.src = 'static/svgs/lock.svg';
  } else {
    contextSecurityLabel.textContent = 'Unlock';
    contextSecurityIcon.src = 'static/svgs/unlock.svg';
  }
  ...
  const firstEnabled = tabContextMenu.querySelector('button:not(:disabled)');
  firstEnabled?.focus();
}

async function runContextSecurityAction() {
  const tab = state.tabs.find((item) => item.id === contextTabId);
  closeTabContextMenu();
  if (!tab || isInbox(tab)) return;
  if (!isEncryptedTab(tab)) await setPassword(tab.id);
  else if (!isUnlocked(tab)) await unlockTab(tab.id);
  else await lockTab(tab.id);
}
...
function updateHeaderActions(normals = normalTabCount()) {
  ...
  if (reserved) return;
  protectTabIcon.src = isEncryptedTab(tab) && !isUnlocked(tab) ? 'static/svgs/unlock.svg' : 'static/svgs/lock.svg';
  protectTabButton.title = isEncryptedTab(tab) ? (isUnlocked(tab) ? 'Password options' : 'Unlock tab') : 'Set password';
...
function toggleOverflowMenu() {
  ...
  if (opening) startGuideButton.focus();
}
...
function isValidBackup(backup) {
  return backup && backup.format === BACKUP_FORMAT && backup.version === BACKUP_VERSION && isValidState(backup.state);
}
...
async function importBackupFile(file) {
  ...
  const affectedIds = new Set([...state.tabs.map((tab) => tab.id), ...nextState.tabs.map((tab) => tab.id)]);
  ...
  await displayActiveTab();
  showToast(
    choice === 'replace' ? 'Backup restored' : `${importedCount} ${importedCount === 1 ? 'tab' : 'tabs'} imported`,
  );
}
...
contextSettingsButton.addEventListener('click', () => {
  const tabId = contextTabId;
  closeTabContextMenu();
  if (tabId) renameTab(tabId).catch(console.error);
});
contextSecurityButton.addEventListener('click', () => runContextSecurityAction().catch(console.error));
contextDeleteButton.addEventListener('click', () => {
  const tabId = contextTabId;
  closeTabContextMenu();
  if (tabId) deleteTab(tabId).catch(console.error);
});
tabContextMenu.addEventListener('click', (event) => event.stopPropagation());
tabContextMenu.addEventListener('contextmenu', (event) => event.preventDefault());
tabContextMenu.addEventListener('keydown', (event) => {
  if (event.key !== 'ArrowDown' && event.key !== 'ArrowUp') return;
  event.preventDefault();
  const items = [...tabContextMenu.querySelectorAll('button:not(:disabled)')];
...
document.addEventListener('keydown', (event) => {
  ...
  } else if (event.key === 'Escape' && !tabContextMenu.hidden) {
    event.preventDefault();
    closeTabContextMenu({ returnFocus: true });
  }
});
